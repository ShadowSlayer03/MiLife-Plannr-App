...
const ProductItem: React.FC<ProductItemProps> = ({
  ...
  return (
    <View className="flex-row justify-between items-start p-3 bg-white rounded-xl mb-2 shadow">
      <View className="flex-1 mr-2">
        <Text
          className="text-lg font-kanit-semibold"
          numberOfLines={isSelected ? undefined : 1} // allow multiple lines if selected
          ellipsizeMode={isSelected ? undefined : "tail"} // truncate when not selected
        >
          {isSelected ? product.name : truncateText(product.name, 20)}
        </Text>
